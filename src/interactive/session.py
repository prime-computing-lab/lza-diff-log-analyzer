"""
Interactive session with RAG-enhanced capabilities
"""
...
try:
    from ..rag import LangChainRAGSystem
    RAG_AVAILABLE = True
except ImportError:
    RAG_AVAILABLE = False
...
class InteractiveSession:
    ...
    async def start(self, input_dir: str, analysis_results: Optional[ComprehensiveAnalysisResult] = None):
        ...
        # Index diff files if RAG is enabled
        if self.rag_enabled:
            await self._index_diff_files()
        
        # Start Q&A loop
        await self._simple_qa_loop()
    
    def _show_welcome(self):
        """Show welcome message with RAG capabilities"""
        if RAG_AVAILABLE:
            welcome = """
🤖 **LZA Analysis Assistant** (RAG-Enhanced)

I can help you understand your CloudFormation changes with access to actual diff file content.

**Enhanced capabilities:**
• Direct access to diff file contents for precise answers
• Semantic search across all stack changes
• Detailed IAM change analysis with source citations

**Example questions:**
• "What specific IAM changes were made to the SecurityStack?"
• "Show me the exact resource deletions in us-east-1"
• "What are the network changes in the NetworkStack diff?"
• "Find all changes related to KMS keys"

**Commands:** `help`, `summary`, `index`, `stats`, `exit`
            """
        else:
            welcome = """
...
💡 **Tip:** Activate the UV virtual environment for enhanced capabilities: `source .venv/bin/activate`
            """
    ...
    async def _init_rag_system(self):
        """Initialize RAG system if available and configured."""
        if not RAG_AVAILABLE:
            self.console.print("ℹ️ RAG components not available")
            return
            
        try:
            # Get the raw config from YAML to access RAG settings
            from ..llm.config import ConfigLoader
            import yaml
            ...
            if not rag_config.get("enabled", False):
                self.console.print("ℹ️ RAG system disabled in configuration")
                return
                
            self.console.print("🔄 Initializing RAG system...")
            self.rag_system = LangChainRAGSystem(raw_config)
            self.rag_enabled = True
            self.console.print("✅ RAG system initialized with LangChain")
            
        except ImportError as e:
            self.console.print(f"❌ RAG dependencies missing: {e}")
            self.console.print("💡 Activate UV environment: source .venv/bin/activate")
        except Exception as e:
            self.console.print(f"❌ RAG initialization failed: {e}")
            import traceback
            traceback.print_exc()
    
    async def _index_diff_files(self):
        """Index diff files for RAG retrieval."""
        if not self.rag_enabled or not self.input_dir:
            return
            
        try:
            input_path = Path(self.input_dir)
            if not input_path.exists():
                self.console.print(f"❌ Input directory not found: {self.input_dir}")
                return
                
            self.console.print("🔄 Indexing diff files for semantic search...")
            
            stats = self.rag_system.index_diff_directory(input_path)
            
            if stats["indexed_files"] > 0:
                self.console.print(f"✅ Indexed {stats['indexed_files']} diff files")
            elif stats["skipped_files"] > 0:
                self.console.print(f"ℹ️ {stats['skipped_files']} files already indexed")
            else:
                self.console.print("⚠️ No diff files found to index")
                
            if stats["failed_files"] > 0:
                self.console.print(f"❌ Failed to index {stats['failed_files']} files")
                
        except Exception as e:
            self.console.print(f"❌ Indexing failed: {e}")
            import traceback
            traceback.print_exc()
            self.rag_enabled = False
    ...
    async def _answer_question(self, question: str):
        """Answer a question using RAG-enhanced LLM or fallbacks"""
        self.console.print("🤔 Thinking...")
        
        # Try RAG-enhanced LLM first if available
        if self.rag_enabled and self.llm_provider:
            try:
                answer = await self._get_rag_enhanced_answer(question)
                if answer:
                    self._display_answer(answer, "RAG-Enhanced AI")
                    return
            except Exception as e:
                self.console.print(f"[yellow]RAG AI error: {e}[/yellow]")
    ...
    async def _get_llm_answer(self, question: str) -> Optional[str]:
        ...
        system_prompt = """You are an AWS Landing Zone Accelerator (LZA) expert analyzing CloudFormation diff logs from an LZA upgrade.
...
IMPORTANT: Maintain consistency with our conversation history. If you previously discussed specific changes, reference them appropriately.
...
Please provide a helpful answer based on the analysis data."""
    ...
    async def _get_rag_enhanced_answer(self, question: str) -> Optional[str]:
        ...
        try:
            ...
            # Apply contextual compression to documents
            compressed_docs = self._compress_documents(docs, question)
            
            # Format retrieved context
            context_text = self._format_retrieved_context(compressed_docs)
            
            # Create enhanced context summary with conversation history
            context_summary = self._create_rag_enhanced_context(question, context_text)
            
            # Perform semantic analysis on IAM changes if detected
            semantic_analysis = self._perform_semantic_analysis(question, context_text)
...
Please answer based on the specific diff content provided above and maintain consistency with our conversation history."""
            ...
            self.console.print(f"[yellow]RAG processing error: {e}[/yellow]")
            return None
    ...
    def _format_retrieved_context(self, docs: List) -> str:
        ...
        for i, doc in enumerate(docs[:5]):  # Limit to top 5 results
            metadata = doc.metadata
            source_info = f"File: {metadata.get('filename', 'unknown')}"
            if metadata.get('stack_name'):
                source_info += f" | Stack: {metadata.get('stack_name')}"
            if metadata.get('chunk_type'):
                source_info += f" | Section: {metadata.get('chunk_type')}"
                
            context_parts.append(f"--- Document {i+1} ---")
            context_parts.append(source_info)
            
            # For IAM content, include more content to capture full tables
            content_limit = 3000 if metadata.get('section') == 'iam_statement_table' or 'iam' in metadata.get('chunk_type', '').lower() else 1000
            content = doc.page_content[:content_limit]
            
            # If content was truncated and it's IAM content, add truncation note
            if len(doc.page_content) > content_limit:
                content += f"\n\n[Content truncated - {len(doc.page_content) - content_limit} more characters available]"
    ...
    def _perform_semantic_analysis(self, question: str, context_text: str) -> Optional[str]:
        """Perform semantic analysis on IAM changes if detected in context"""
        # Check if this is an IAM-related question
        if not any(keyword in question.lower() for keyword in ['iam', 'policy', 'permission', 'role', 'dependenciesstack']):
            return None
        
        # Check if context contains IAM changes
        if 'iam statement changes' not in context_text.lower():
            return None
        
        # For now, return guidance text - could be enhanced to parse actual changes
        return """IAM ANALYSIS GUIDANCE:
- Look for ADD/REMOVE pairs that affect the same resource
- String-to-array conversions (e.g., "value" → ["value"]) are FORMAL changes with no security impact
- Focus on whether permissions actually change, not just the format
- Condition changes should be compared semantically, not structurally"""
    
    def _build_enhanced_system_prompt(self, question: str) -> str:
        """Build enhanced system prompt based on question type"""
        # Determine the type of analysis needed
        if any(keyword in question.lower() for keyword in ['dependenciesstack', 'iam']):
            base_prompt = self.iam_analysis_prompts.get_iam_analysis_prompt("dependencies")
        else:
            base_prompt = self.iam_analysis_prompts.get_iam_analysis_prompt()
        
        # Add formal change examples for IAM questions
        if 'iam' in question.lower():
            base_prompt += "\n\n" + self.iam_analysis_prompts.get_formal_change_examples()
        
        # Add comparative analysis instructions
        base_prompt += "\n\n" + self.iam_analysis_prompts.get_comparative_analysis_prompt()
        
        # Add conversation context if available
        if self.conversation_history:
            context_summary = self._get_conversation_context_summary()
            if context_summary:
                base_prompt += f"\n\nConversation Context:\n{context_summary}"
                
        return base_prompt
    
    def _create_rag_enhanced_context(self, question: str, retrieved_content: str) -> str:
        """Create enhanced context with retrieved content."""
        base_context = self._create_simple_context(question)
...
"""
    ...
    async def _reindex_files(self):
        """Re-index diff files."""
        if not self.rag_enabled:
            self.console.print("[yellow]RAG system not enabled[/yellow]")
            return
...
- Unique stacks: {stats.get('unique_stacks', 0)}
- Chunk types: {', '.join(stats.get('chunk_type_distribution', {}).keys())}
            """
            
            panel = Panel(Markdown(stats_text), title="RAG Statistics", border_style="green")
            self.console.print(panel)
            self.console.print()
...
**Recommendation:** {risk_assessment.get('recommendation', 'Review changes carefully')}
    ...
    def _enhance_query_with_context(self, question: str) -> str:
        ...
        recent_context = []
        for i, (prev_q, prev_a) in enumerate(self.conversation_history[-3:]):  # Last 3 Q&A pairs
            if "dependenciesstack" in prev_q.lower() or "iam" in prev_q.lower():
                recent_context.append(f"Previous Q{i+1}: {prev_q}")
                # Extract key terms from previous answer
                if "dependenciesstack" in prev_a.lower():
                    recent_context.append("Context: DependenciesStack IAM changes")
                    
        if recent_context:
            context_text = " ".join(recent_context)
            enhanced_query = f"{question} {context_text}"
            return enhanced_query
            
        return question
    
    def _compress_documents(self, docs: List, question: str) -> List:
        """Apply contextual compression to documents for more relevant content."""
        if not docs:
            return docs
        ...
        for doc in docs:
            ...
            # For IAM-related questions, focus on IAM table content
            if "iam" in question_lower and "iam_statement_table" in metadata.get("section", ""):
                # Extract just the IAM table portion
                lines = content.split('\n')
                table_lines = []
                in_table = False
                
                for line in lines:
                    if "IAM Statement Changes" in line or "Effect" in line and "Action" in line:
                        in_table = True
                    if in_table:
                        table_lines.append(line)
                    if in_table and line.strip() == "" and len(table_lines) > 5:
                        break
            ...
            # For stack-specific questions, focus on relevant sections
            if "dependenciesstack" in question_lower:
                # Extract relevant sections for DependenciesStack
                lines = content.split('\n')
                relevant_lines = []
                
                for i, line in enumerate(lines):
                    if any(keyword in line.lower() for keyword in ["dependenciesstack", "iam", "effect", "action", "principal", "resource"]):
            ...
            # Default: truncate long documents but keep full content for shorter ones
            if len(content) > 2000:
                # Keep first 1000 and last 1000 characters with indicator
                compressed_content = content[:1000] + "\n\n[... middle content compressed ...]\n\n" + content[-1000:]
                from langchain_core.documents import Document
                compressed_doc = Document(
                    page_content=compressed_content,
                    metadata=metadata
    ...
    def _build_conversation_aware_system_prompt(self) -> str:
        """Build system prompt that includes conversation context."""
        base_prompt = """You are an AWS Landing Zone Accelerator (LZA) expert with access to actual CloudFormation diff file contents.

Use the retrieved diff content to provide specific, accurate answers. Always cite the source files when referencing specific changes.

When analyzing IAM changes from diff files:
- Parse IAM Statement Changes tables carefully, noting '+' (additions) and '-' (removals)
- Identify specific resources, effects, actions, principals, and conditions
- Explain what the change means in terms of permissions granted or revoked
- Note any security implications of the changes
- Reference the exact stack and account where changes occur

For all changes:
- Provide exact resource names and types from the diff
- Explain the before/after state when possible
- Reference specific file names and line numbers if available
- Highlight any potential security or operational impacts
...
IMPORTANT: Maintain consistency with our conversation history. If you previously discussed specific changes, reference them appropriately."""
    ...
    def _get_conversation_context_summary(self) -> str:
        ...
        context_parts = []
        for i, (question, answer) in enumerate(self.conversation_history[-3:]):  # Last 3 Q&A pairs
            # Extract key information from previous Q&A
            if "dependenciesstack" in question.lower():
                context_parts.append(f"Previously discussed: DependenciesStack IAM changes")
            elif "iam" in question.lower():
                context_parts.append(f"Previously discussed: IAM changes")
            elif "risk" in question.lower():
                context_parts.append(f"Previously discussed: Risk assessment")
